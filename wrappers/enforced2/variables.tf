...
variable "table_definition_repo" {
  ...
  validation {
    condition     = length(trim(var.table_definition_repo, " ")) > 0 && can(regex("/", var.table_definition_repo))
    error_message = "table_definition_repo must be set in owner/repo format."
  }
}
...
variable "table_definition_path" {
  ...
  validation {
    condition     = length(trim(var.table_definition_path, " ")) > 0
    error_message = "table_definition_path must be set."
  }
}
